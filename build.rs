...
fn main() {
    ...
    // Extract sanitizer value from RUSTFLAGS (e.g., "address" or "thread")
    let sanitizer = rustflags
        .split_whitespace()
        .find_map(|flag| flag.strip_prefix("sanitizer="));

    let instrumented = coverage_enabled || sanitizer.is_some();

    if instrumented {
        // Coverage and sanitizers need debug info and reduced optimization
        build.flag("-O1").flag("-g");
    }

    if coverage_enabled {
        // Coverage flags are Clang-specific (cc crate uses CXX env var)
        build
            .flag("-fprofile-instr-generate")
            .flag("-fcoverage-mapping");
    } else if let Some(sanitizer) = sanitizer {
        // Forward Rust sanitizer to C++, plus UBSAN (Rust doesn't support UBSAN
        // but C++ does, and it's compatible with ASAN/TSAN)
        build
